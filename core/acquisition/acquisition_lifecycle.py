...
def _validate_parameters(app) -> bool:
    ...
    Checks for selected channels, a valid bin width, and (Laserblood-specific)
    the presence of required Laserblood metadata when data writing is enabled.
    ...
    # Laserblood-specific: require metadata to be filled before writing data
    if (
        app.write_data_gui
...
def _prepare_spectroscopy_parameters(app, frequency_mhz) -> dict:
    ...
    Laserblood: get_firmware_selected is called without the pico_mode kwarg
    (the Laserblood ControlsController does not expose that override).
    ...
    # Laserblood: no pico_mode kwarg override
...
def _handle_post_acquisition_tasks(app):
    """
    Handles final tasks after acquisition stops.

    Laserblood-specific: also updates FPGA firmware, average CPS and SBR
    in the LaserbloodMetadataPopup after saving data.
    ...
    # Laserblood-specific metadata updates after each acquisition
...
def check_card_connection(app, start_experiment=False):
    ...
    Raises:
        Exception: Re-raises if start_experiment is True and card not found.
    """
    try:
