...
- pull_from_queue          — main data polling loop
...
def pull_from_queue(app):
    ...
    Dispatches different data types (decay curves, phasors) to the
...
def update_acquisition_countdowns(app, time_ns):
    ...
    Calculates remaining time and updates the UI labels.
    Does nothing if acquisition is in 'Free running' mode.
...
def update_cps(app, channel_index, time_ns, curve):
    ...
    Calculates the CPS based on the incoming data rate, triggers a pile-up
    warning animation if CPS exceeds the user-defined threshold, and updates
    the SBR value if enabled.
...
def acquired_spectroscopy_data_to_fit(app, read):
    ...
    Extracts decay curve data (x and y values) and time shifts for the
    currently displayed channels. Uses get_channel_name() for titles.
        ...
        data.append(
            {
                "x": x_data,
                "y": y,
                # Standard edition: uses channel name from app.channel_names
