...
def draw_lifetime_points_in_phasors(app, channel, harmonic, laser_period_ns, frequency_mhz):
    ...
    tau_m = np.array([
        0.1e-9, 0.5e-9, 1e-9, 2e-9, 3e-9, 4e-9,
        5e-9, 6e-9, 7e-9, 8e-9, 9e-9, 10e-9,
    ])
    ...
    for i in range(len(g)):
        text = pg.TextItem(
            f"{tau_m[i] * 1e9:.1f} ns", anchor=(0, 0), color="white", border=None,
        )
...
def quantize_phasors(app, harmonic, bins=64):
    ...
    for i, channel_index in enumerate(app.plots_to_show):
        ...
        if channel_index in app.quantization_images:
            app.phasors_widgets[channel_index].removeItem(app.quantization_images[channel_index])
        if channel_index in app.phasors_colorbars:
            app.phasors_widgets[channel_index].removeItem(app.phasors_colorbars[channel_index])
