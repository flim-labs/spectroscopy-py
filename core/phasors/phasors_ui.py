...
def clear_phasors_files_legend(app):
    ...
    Spectroscopy base: hides legends via setVisible(False) rather than
    removing them from the widget entirely.
    """
        ...
        if is_phasors_read_mode:
            ...
            for file_name, coords in points_by_file.items():
                ...
                scatter = pg.ScatterPlotItem(
                    [mean_g], [mean_s], size=20,
                    pen={"color": "#0066CC", "width": 4}, symbol="x",
                )
                scatter.setZValue(3)
            ...
            scatter = pg.ScatterPlotItem(
                [mean_g], [mean_s], size=20,
                pen={"color": "yellow", "width": 4}, symbol="x",
            )
            scatter.setZValue(2)
            app.phasors_widgets[channel_index].addItem(scatter)
...
def generate_phasors_legend(app, harmonic):
    ...
    for i, channel_index in enumerate(app.plots_to_show):
        if not (hasattr(app, 'phasors_legend_labels') and channel_index in app.phasors_legend_labels):
            continue
