...
def _handle_spectroscopy_tab_selection(app):
    ...
    Also shows the N° Replicate control (Laserblood-specific).
    ...
    # Show N° Replicate control in spectroscopy tab (Laserblood-specific)
...
def _handle_fitting_tab_selection(app):
    ...
    Hides N° Replicate in read mode, shows it in acquire mode (Laserblood-specific).
    ...
    # Replicate control visibility depends on read/acquire mode (Laserblood-specific)
...
def _handle_phasors_tab_selection(app):
    ...
    Hides N° Replicate only in read mode (Laserblood-specific).
    ...
    # Replicate control: hide only in phasors read mode (Laserblood-specific)
    if app.acquire_read_mode == "read":
        app.control_inputs[s.SETTINGS_REPLICATES].setVisible(False)
        app.control_inputs["replicates_label"].setVisible(False)
    else:
