...
operations to the dedicated sub-modules in `core/plots`.

Sub-module responsibilities
---------------------------
core.plots.plot_initializers  — numpy data initialization for intensity and decay plots
core.plots.plot_builders      — pyqtgraph widget construction for all plot types
                          (intensity section, decay curve, spectroscopy panel,
                           phasor panel); wires widgets into app state dicts.
                          LaserBlood-specific: also populates app.intensity_plot_widgets
                          and uses app.channel_names directly.
...
class PlotsController:
    ...
    `core/plot_*.py` sub-module. Orchestration methods that coordinate across
    modules or manage top-level app state (generate_plots, clear_plots,
    set_plot_y_range) remain implemented here.

    LaserBlood-specific: clear_plots also clears app.intensity_plot_widgets,
    which stores direct PlotWidget references populated by create_intensity_section.
    """
