...
import settings.laserblood_settings as ls
...
class SpectroscopyWindow(QWidget):
    ...
    def _initialize_settings(self):
        ...
        self.laserblood_laser_type = self.settings.value(
            ls.SETTINGS_LASER_TYPE, ls.DEFAULT_LASER_TYPE
        )
        self.laserblood_filter_type = self.settings.value(
            ls.SETTINGS_FILTER_TYPE, ls.DEFAULT_FILTER_TYPE
        )
        self.laserblood_new_added_inputs = json.loads(
            self.settings.value(
                ls.NEW_ADDED_LASERBLOOD_INPUTS_KEY, ls.NEW_ADDED_LASERBLOOD_INPUTS_JSON
            )
        )
        ...
        self.pico_mode = False # Pico mode always set to False; remove 100ps firmware support
