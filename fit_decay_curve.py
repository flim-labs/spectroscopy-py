...
def multiprocessing_target(
    result, x_values, y_values, channel, y_shift, tau_similarity_threshold
):
    try:
        result["value"] = fit_decay_curve(
            x_values, y_values, channel, y_shift, tau_similarity_threshold
        )
    except Exception as e:
        result["error"] = str(e)


def fit_decay_curve_multiprocessing(
    x_values, y_values, channel, y_shift=0, tau_similarity_threshold=0.01, timeout=30
):
    manager = multiprocessing.Manager()
    result = manager.dict()
    process = multiprocessing.Process(
        target=multiprocessing_target,
        args=(result, x_values, y_values, channel, y_shift, tau_similarity_threshold),
    )
    process.start()
    ...
    if process.is_alive():
        process.terminate()
        process.join()
        raise TimeoutError("Timeout error: calculation took to much time.")
    if "error" in result:
...
def fit_decay_curve(
    ...
    for model, initial_guess in decay_models:
        try:
            popt, pcov = curve_fit(
                model, t_data, y_data, p0=initial_guess, maxfev=500000
            )
            ...
            epsilon = 1e-10
            chi2 = np.sum(
                (observed_values - expected_values) ** 2 / (expected_values + epsilon)
            )
